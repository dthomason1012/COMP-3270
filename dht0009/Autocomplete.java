...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         for (int i = 0; i < word.length(); i++) {
            if (!curr.children.containsKey(word.charAt(i))) {
               Node temp = new Node (word.charAt(i), curr, weight);
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         PriorityQueue<Term> terms = new PriorityQueue<Term>(new Term.ReverseWeightOrder());
         ...
         while (!nodes.isEmpty()) {
            current = nodes.poll();
            nodes.addAll(current.children.values());
            if (current.isWord) {
               Term term = new Term(current.myWord, current.myWeight);
               terms.add(term);
            }
         }
         for (Term t : terms) {
      ...
      public double weightOf(String term) {
         ...
         for (int i = 0; i < term.length(); i++) {
            if (n.children.containsKey(term.charAt(i))) {
               n = n.children.get(term.charAt(i));
            }
            else {
               return 0.0;
            }
         }
         return (double) n.getWeight();
      }
